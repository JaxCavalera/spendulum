...
/**
 * Recursive function that takes a target item and returns a deep clone of it that has no linked references to the original
...
export const deepClone = (target: any) => {
  ...
  const iterableKeys = Object.keys(newTarget).filter(key => {
    return Array.isArray(newTarget[key]) || isRealObject(newTarget[key]);
  });

  while (iterableKeys.length) {
    const nextKey = <string>iterableKeys.shift();
